...
class PolnocvetsModelPlotnost extends JModel
{
    var $_data;
    var $_id;

    function delData($id){
        $table = $this->getTable('plotnost', '');
        if (!$table->delete( $id )) {
    ...
    function saveData($id,$save)
    {
        ...
        if ($id <= 0 ) {
            $query = ' SELECT * FROM `jos_zepp_polnocvet_plotnost` WHERE `name` LIKE \''.$save['name'].'\' ';;
    ...
    {
        $query = ' SELECT * '
            . ' FROM #__zepp_polnocvet_plotnost '
            ;
