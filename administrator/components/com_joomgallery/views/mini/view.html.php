...
// $Id: view.html.php 3327 2011-09-23 16:27:15Z aha $
/****************************************************************************************\
**   JoomGallery  1.5.7                                                                 **
...
class JoomGalleryViewMini extends JoomGalleryView
{
  ...
  function display($tpl = null)
  {
    ...
    $url   = JRoute::_('index.php?option='._JOOM_OPTION.'&view=mini&format=raw', false);
    $lists['image_categories']         = JHTML::_('joomselect.categorylist', $catid, 'catid', 'onchange="javascript:ajaxRequest(\''.$url.'\', 0, \'catid=\' + this[this.selectedIndex].value)"');
    ...
    if($extended > 0)
    {
      ...
      // Create Category
      $lists['parent_categories'] = JHTML::_('joomselect.categorylist', 0, 'parent');
      $this->_ambit->script('JG_COMMON_ERROR_CATEGORY_MUST_HAVE_TITLE');
    }
    ...
    $this->_ambit->script('JGA_MINI_PLEASE_ENTER_TEXT');
