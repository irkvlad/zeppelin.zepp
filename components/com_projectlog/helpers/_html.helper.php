...
class projectlogHTML
{
	...
	function catSelect($tag, $attrib, $sel = null)
	{
		$database = JFactory::getDBO();
		$cats     = array();
		//$cats[] = JHTML::_('select.option', '6', JText::_('CATEGORYS'));
		...
		return JHTML::_('select.genericlist', $cats, $tag, $attrib, 'value', 'text', $sel);//$cats;
	}
	...
	*/
	function notifyAdmin($type, $user, $project, $onLoadM)
	{
		global $mainframe;
		...
		switch ($type)
		{
			...
			case 'brak':
				if ($type and $proect->category == 12 and $proect->location_spec <> '')
				{
					...
					// $db = JFactory::getDBO();
//	               $query ="INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( ".$user->id." ,  '".$body."' , ".$proect->manager." , 1 , ".$proect->id. " ) ;";
//		    	   $db->setQuery($query);
//		           if(!$db->query()) {
...
//
//                    $db = JFactory::getDBO();
//	               $query ="INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( ".$user->id." ,  '".$body."' , 87 , 1 , ".$proect->id. " ) ;";
//		    	   $db->setQuery($query);
//		           if(!$db->query()) {
//				        $this->setError($db->getErrorMsg());
//				        //return false;
...
//
//                   $db = JFactory::getDBO();
//	               $query ="INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( ".$user->id." ,  '".$body."' , 97 , 1 , ".$proect->id. " ) ;";
//		    	   $db->setQuery($query);
//		           if(!$db->query()) {
//				        $this->setError($db->getErrorMsg());
//				        //return false;
//			       }
//                   $db = JFactory::getDBO();
//	               $query ="INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( ".$user->id." ,  '".$body."' , ".$proect->technicians." , 1 , ".$proect->id. " ) ;";
//		    	   $db->setQuery($query);
//		           if(!$db->query()) {
//				        $this->setError($db->getErrorMsg());
//				        //return false;
//			       }
	...
	function notifyDoc($type, $user, $project_id, $faile)
	{
		...
		if ($type)
		{
			...
			if ($uname <> $t_email and $t_email <> '' and ($proect->category == 7 or $proect->category == 8 or $proect->category == 12 or $proect->category == 13))
			{
				...
				// Для ZeppProjekt
				$db    = JFactory::getDBO();
				$query = "INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( " . $user->id . " ,  '" . $body . "' , " . $proect->technicians . " , " . $pochta_chek . " , " . $idd . " ) ;";
				$db->setQuery($query);
				if (!$db->query())
				{
					$this->setError($db->getErrorMsg());
	...
	*/
	function notifyUsers($project_id, $mov, $msg)
	{
		...
		if ($project_id)
		{
			...
			$db    = JFactory::getDBO();
			$query = "SELECT cherep , pochta , move , comment FROM #__users WHERE id = " . $proect->manager;
			$db->setQuery($query);
			$chekM = $db->loadObject();
			...
			$db->setQuery($query);
			$dok      = $db->loadObjectlist();
			$doc_path = 'http://zeppelin/zepp/media/com_projectlog/docs/';//'\\\\Zeppelin\\nash-zepp\\zepp\\media\\com_projectlog\\docs\\';
	...
	*/
				...
				case '13':
				{
					...
					if (projectlogHTML::getUserPChekc($proect->manager) == 1)
						JUtility::sendMail('baza@zepp.ru', 'Дизайн-студия ЦеППелин', $user_email, $subject, $body, $mode, $cc, $bcc, $attachment);
					//if ($proect->podrydchik !='')
					//  JUtility::sendMail('baza@zepp.ru', 'Дизайн-студия ЦеППелин', 'black@zepp.ru', $subject, $body, $mode, $cc, $bcc, $attachment);

					// Для ZeppProjekt
					/*   $db = JFactory::getDBO();
					  $query ="INSERT INTO #__zepp_pochta ( from_user_id , text , to_user_id , tema , project_id) VALUES ( 97 ,  '".$body."' , ".$proect->manager." , 1 , ".$project_id. " ) ;";
					  $db->setQuery($query);
					  if(!$db->query()) {
						   $this->setError($db->getErrorMsg().$moveDate.$category+1);
						   //return false;
					 ...
					 $db = JFactory::getDBO();
					  $query = 'SELECT cherep  FROM #__users WHERE id = '.$proect->manager;
					  $db->setQuery($query);
					  $chek =$db->loadResult();

					  $query ="UPDATE #__users SET cherep = ".($chek + 1)." WHERE  id = ".$proect->manager ;
					  $db->setQuery($query);
					  if(!$db->query()) {
						  $this->setError($db->getErrorMsg().$moveDate.$category+1);
						  //return false;
