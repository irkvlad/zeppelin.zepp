...
class  clientViewClient extends JView
{
	function display($tpl = null)
	{
		...
		//$rows = $this->get('ClientContact');
		 ...
		 if( ($send==1) OR ($SortManager > 0 ) ) $client = $this->get('send');
			 ...
			 if(!empty($search)){
				...
				$search = '<font size="3" color="#FF0000" style="background-color:yellow;" face="Arial">&nbsp;'.$search;
					if( !empty($rows) ) {$search.="&nbsp;</font> - ".count($rows)." вхож."; $client = $rows;}
					else { $client = $this->get('Client'); $search.="</font> - не найдено";}
		...
		$this->assignRef('Managers', $IdManagers);
