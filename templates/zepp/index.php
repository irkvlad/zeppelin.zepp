...
$view=JRequest::getVar( 'view', 'NONE');
$id=JRequest::getVar( 'id','NONE' );
$catid=JRequest::getVar( 'catid','NONE' );
$date2 = '25.06.2016';
...
<div class="clientmod">
    <jdoc:include type="modules" name="clientmod" />
</div>
<?php //endif; ?>
...
<!-- Показываем счетчик на определенной странице -->
<?php /* if ($view=='article' AND $id=='277') {
...
//$arr1 = explode('.', $date1);
$arr2 = explode('.', $date2);
$time1 = mktime(0, 0, 0, date("m")  , date("d"), date("Y"));//mktime(0,0,0,$arr1[1],$arr1[0],$arr1[2]);
$time2 = mktime(0,0,0,$arr2[1],$arr2[0],$arr2[2]);
$dif = ($time2 - $time1) / 86400;
$d = substr($dif, strlen($dif) - 1, strlen($dif));

echo '<center><h1>'.$dif;
if ($dif > 9 AND $dif < 19 ) { echo ' дней'; } 
else {
	if ($d == 1) echo ' день';
	if ($d > 1 AND $d < 5 ) echo ' дня';
	if ($d > 4) echo ' дней';
}

echo ' </h1></center>';
 
 echo '<h2><a href="http://192.168.5.200/zepp/index.php?option=com_content&view=category&layout=blog&id=6&Itemid=39">Наши новости</a></h2>';
